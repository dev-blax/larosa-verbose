...
class _CenterSnapCarouselState extends State<CenterSnapCarousel> {
    ...
    if (controller != null) {
      setState(() {
        if (controller.value.isPlaying) {
          controller.pause();
          _manualControlStates[index] = false;
          LogService.logInfo("Video at index $index paused via manual control.");
        } else {
          controller.play();
          _manualControlStates[index] = true;
          LogService.logInfo("Video at index $index playing via manual control.");
        }
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    const offset = 0.0;
    // Compute logical post height based on widget.postHeight (if provided) converted from physical pixels
    final logicalPostHeight = ((widget.postHeight != null
                ? widget.postHeight! / MediaQuery.of(context).devicePixelRatio
                : 300.0) +
            offset)
      ...
      child: SingleChildScrollView(
        ...
        child: Stack(
          children: [
            ...
            // Media position indicator (only shown if multiple media items)
            if (widget.mediaUrls.length > 1)
              Positioned(
                top: 16,
                right: 16,
  ...
  /// This method is used to build a media container for a given image or video.
  /// Debug logs have been added here as well to track the container’s sizes.
  Widget buildMediaContainer({
    required String? imageUrl,
    required CachedVideoPlayerPlusController? videoController,
    required BuildContext context,
    ...
    const double containerHeight = 300;
    LogService.logInfo("Building media container. isVideo: $isVideo, containerHeight: $containerHeight");
    return ConstrainedBox(
      constraints: const BoxConstraints(
        maxHeight: containerHeight,
      ),
      child: Container(
        ...
        child: isVideo
            ...
            : CachedNetworkImage(
                imageUrl: imageUrl!,
                placeholder: (context, url) => _buildShimmerLoader(height: containerHeight),
                errorWidget: (context, url, error) {
                  LogService.logError("Error loading image in media container: $error");
                  return const Icon(Icons.error, size: 50, color: Colors.redAccent);
                },
              ),
      ),
    );
