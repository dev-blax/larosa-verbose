...
class FavouritesComponent extends StatefulWidget {
  final int profileId;
  const FavouritesComponent({super.key, required this.profileId});
...
class _FavouritesComponentState extends State<FavouritesComponent> {
  ...
  Future<void> _fetchLikedPosts() async {
    ...
    var url = Uri.https(
      ...
      '/feeds/fetch/favorites',
    );

    try {
      LogService.logDebug('fetching favorites');
      ...
      if (response.statusCode != 200) {
        LogService.logError('Error fetching favorites: $response.statusCode');
        return;
      }

      LogService.logInfo('Success fetching favorites');
